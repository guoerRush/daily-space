...
本项目不能直接部署到“扣子 AI 办公助手”的聊天页。该页面是聊天工具，不提供 GitHub 导入、Docker 构建、网站域名或常驻后台服务。若使用扣子云，需要从“创作 -> 扣子编程”进入项目空间，由扣子编程的云部署能力导入仓库并创建服务。

需要选择支持 Docker 和常驻服务的云平台。扣子云若提供这两项能力，可以按本文部署；否则应选择腾讯云 CloudBase、腾讯云轻量应用服务器或阿里云 ECS。部署时需要创建两个服务，二者都从同一个 GitHub 仓库读取代码。
