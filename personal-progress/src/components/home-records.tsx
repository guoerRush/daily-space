...
export function HomeRecords() {
  ...
  return (
    <>
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <div className="flex items-center gap-2 text-primary">
              ...
              <CardTitle>记录此刻</CardTitle>
            </div>
            <CardDescription>用三个动作完成今天的记录与复盘。</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-2">
            ...
            <Button
              className="w-full"
              variant="outline"
              render={<Link href={`/journals/${todayKey()}#reflection`} />}
            >
              写下今日反思
            </Button>
            <Button
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="grid gap-2 sm:grid-cols-3">
            <HistoryAction
              ...
              expanded={expanded === "journals"}
              onToggle={() => toggle("journals")}
            />
            <HistoryAction
              title="成功日记"
      ...
      <SuccessJournalDialog
        key={successSession}
        open={successOpen}
        initialContent={successDraft}
...
function TodayPlanCard({ entry }: { entry: PlanEntry | undefined }) {
  ...
  const dictationComplete = Boolean(plan?.dictation && richTextToPlainText(plan.dictation).trim());
  const hasContent =
    tasks.length ||
    planned.length ||
    actual.length ||
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        ...
        <CardDescription>仅展示今天的待办、计划与执行情况。</CardDescription>
      </CardHeader>
      <CardContent>
        {hasContent ? (
          <div className="flex flex-col gap-3">
            ...
            {planned.length ? (
              <div>
                <p className="text-sm font-medium">计划完成</p>
            ...
            {actual.length ? (
              <div>
                <p className="text-sm font-medium">实际完成</p>
                <div className="mt-2 flex flex-col gap-1 text-sm text-muted-foreground">
                  {actual.map((row) => (
                    <p key={row.id}>
                      {row.time ? `${row.time} · ` : ""}
        ...
        <div className="mt-4 flex items-center justify-between border-t border-[#dfe5df] pt-3 text-sm">
          <span className="font-medium text-[#526158]">英语听写</span>
          <span className={dictationComplete ? "text-[#2f6651]" : "text-[#8a968e]"}>{dictationComplete ? "已完成" : "未完成"}</span>
        </div>
        <Button
          className="mt-4"
          variant="outline"
...
function HistoryAction({
  ...
  return (
    <Button
      className="h-auto min-h-16 justify-between px-3 py-3"
      variant="outline"
      onClick={onToggle}
...
function HistoryPanel({
  ...
  return (
    <section className="mt-4 border border-border bg-card shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          <h2 className="font-semibold">{config.title}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            共 {count} 条，按日期从近到远完整保留。
          </p>
        </div>
        {config.href ? (
          <Button variant="ghost" render={<Link href={config.href} />}>
            打开完整页面
          </Button>
        ) : null}
      </div>
      {count ? (
        <div className="max-h-[36rem] overflow-y-auto">
          {kind === "journals"
            ...
            : kind === "success"
              ? successes.map((entry) => (
                  <SuccessRow key={entry.date} entry={entry} />
                ))
              : notes.map((entry) => <NoteRow key={entry.id} entry={entry} />)}
        </div>
      ) : (
...
function JournalRow({ entry }: { entry: JournalEntry }) {
  return (
    <article className="border-b border-border px-5 py-5 last:border-0">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="font-medium">{formatJournalDate(entry.date)}</h3>
        <Link
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        心情：{entry.mood}
        {entry.images?.length ? ` · ${entry.images.length} 张图片` : ""}
      </p>
      <RichTextContent
        value={entry.content}
...
function NoteRow({ entry }: { entry: Note }) {
  return (
    <article className="border-b border-border px-5 py-5 last:border-0">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="font-medium">{entry.title || "未命名随记"}</h3>
          <p className="mt-1 text-xs text-muted-foreground">
            {categoryInfo[entry.category].label} ·{" "}
...
function SuccessJournalDialog({
  ...
  useEffect(() => {
    if (!open || !richTextToPlainText(content)) return;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>今天的成功日记</DialogTitle>
          <DialogDescription>
            把成果、坚持、突破或微小的进步留下来。每 3 秒自动保存一次。
          </DialogDescription>
        </DialogHeader>
        <RichTextEditor
          value={content}
          onChange={setContent}
          placeholder="例如：我按计划完成了学习，并在疲惫时仍然认真写下了复盘。"
