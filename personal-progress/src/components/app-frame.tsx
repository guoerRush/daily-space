...
export function AppFrame({ children }: { children: ReactNode }) {
  ...
  return <AuthGate><div className="min-h-screen bg-[#f7f9f6] text-[#18241d]">
    <header className="sticky top-0 z-20 border-b border-[#dce5dd] bg-white/85 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-4 md:px-8">
        ...
        <div className="flex items-center gap-1.5"><DailyStatus /><VoiceDictationButton /><AccountControl /><Link href="/settings#reminders" aria-label="提醒设置" title="提醒设置" className="relative grid h-9 w-9 place-items-center rounded-lg text-[#526158] hover:bg-[#edf4ee]"><Bell size={18} /><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-[#c76b58]" /></Link><Link href="/settings" aria-label="设置" title="设置" className="grid h-9 w-9 place-items-center rounded-lg text-[#526158] hover:bg-[#edf4ee]"><Settings size={18} /></Link></div>
      </div>
    </header>
    <DailyQuote />
